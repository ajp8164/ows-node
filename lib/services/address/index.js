...
var AddressService = function(options) {
  ...
  this.Unit = require(options.nodeKind + '-lib').Unit;
...
AddressService.prototype.getAddressSummary = function(address, options, callback) {
  ...
  txStream.on('end', function() {
    result.balance = self.Unit.fromSatoshis(result.balanceSat).toBTC();
    assert(result.balance >= 0, 'Balance can\'t be less than zero.');
    result.totalReceived = self.Unit.fromSatoshis(result.totalReceivedSat).toBTC();
    result.totalSent = self.Unit.fromSatoshis(result.totalSentSat).toBTC();
...
AddressService.prototype.getAddressUnspentOutputs = function(address, options, callback) {
  ...
  utxoStream.on('data', function(data) {
    ...
    results.push({
      ...
      amount: self.Unit.fromSatoshis(value.satoshis).toBTC(),
      height: value.height,
