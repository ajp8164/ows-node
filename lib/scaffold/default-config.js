...
/**
 * Will return the path and default ows-node configuration. It will search for the
 * configuration file in the "~/.ows" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.ows');
  var defaultConfigFile = path.resolve(defaultPath, './ows-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    console.log(`The configuration file at '${defaultConfigFile}' is incompatible with this version of OWS Node.`);

    var now = new Date();
    // ows-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `ows-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
