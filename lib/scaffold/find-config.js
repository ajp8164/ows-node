...
/**
 * Will return the path and ows-node configuration
...
function findConfig(cwd) {
  ...
  while (!fs.existsSync(path.resolve(directory, 'ows-node.json'))) {
  ...
  return {
    path: directory,
    config: require(path.resolve(directory, 'ows-node.json'))
  };
}
