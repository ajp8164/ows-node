...
/**
 * Will remove a service from ows-node.json
...
/**
 * Will remove the Node.js service and from the ows-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The ows-node configuration directory
...
function remove(options, done) {
  ...
  var owsnodeConfigPath = path.resolve(configPath, 'ows-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(owsnodeConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a ows-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to ows-node.json
        removeConfig(owsnodeConfigPath, service, next);
      });
    }, done
  );
}
